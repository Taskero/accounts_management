defmodule AccountsManagementAPIWeb.AccountController do
  ...
  def show(conn, %{"id" => id}) do
    with {:ok, %Account{} = account} <- Users.get_account(id) do
      render(conn, :show, account: account)
    end
  end

  def update(conn, %{"id" => id, "account" => account_params}) do
    with account_params <- account_params |> Map.delete("status"),
         account_params <- account_params |> Map.delete("confirmed_at"),
         {:ok, account} <- Users.get_account(id),
  ...
  def delete(conn, %{"id" => id}) do
    with {:ok, account} <- Users.get_account(id),
